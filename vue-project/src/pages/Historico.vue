...
interface Purchase { id: string; cafe_qtd: number; filtro_qtd: number; data_compra: string }
...
async function fetchPurchases() {
  try {
    const res = await api.get(`/compras?usuario_id=${currentUser.id}`);
    purchases.value = res.data || [];
  } catch (err) {
    console.error('Erro ao buscar histórico usuário:', err);
  }
}
